...
class Category(models.Model):
    ...
    CharField 指定了 name 的数据类型，CharField 是字符型，
    max_length 指定其最大长度，超过这个长度的分类名就不能被存入数据库。
    """
...
class Post(models.Model):
    ...
    # 文章正文，我们使用了 TextField。比较短的字符串存储可以使用 CharField，
    # 但对于文章的正文来说可能会是一大段文本，因此使用 TextField 来存储大段文本。
    body = models.TextField()

    # 这两个列分表表示了文章的创建时间和最后一次修改时间，存储时间的列用 DateTimeField。
    created_time = models.DateTimeField()
    modified_time = models.DateTimeField()

    # 文章摘要，可以没有文章摘要，但默认情况下 CharField 要求我们必须存入数据，
    # 否则就会报错。指定 blank=True 后就可以允许空值了。
